...
export default function Dashboard({ onViewExceptions, onViewMatches, dataSource, onRefresh }) {
  ...
  const { data: forecastData, loading: forecastLoading, error: forecastError, refetch: refetchForecast, getChartData, getSummary } = useForecast({ autoFetch: true })
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      setExceptions(exceptions.data.exceptions || [])
    } catch (err) {
  ...
  return (
    <div className="dashboard" style={{ padding: '20px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          ...
          <div style={{ marginTop: '4px' }}>
            <StatusBadge status={dataSourceStatus} label={dataSourceLabel} size="small" />
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          <Button variant="secondary" size="small" onClick={handleRefresh} icon="🔄">Refresh</Button>
          <Button variant="success" size="small" onClick={onViewMatches} icon="✅">View Matches ({totalMatches})</Button>
          <Button variant="warning" size="small" onClick={onViewExceptions} icon="🚨">View Exceptions ({totalExceptions})</Button>
        </div>
      </div>
      ...
      <div style={{ marginBottom: '16px' }}>
        <ForecastChart data={forecastData} loading={forecastLoading} error={forecastError} onRefresh={refetchForecast} />
      </div>

      {/* Forecast Summary Cards */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '12px', marginBottom: '24px' }}>
        <div style={{ background: '#f8f9fa', padding: '14px 18px', borderRadius: '8px', border: '1px solid #e9ecef', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      ...
      {forecastData?.percentiles && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px', marginBottom: '24px' }}>
          <div style={{ background: '#ff9800', color: 'white', padding: '12px 16px', borderRadius: '8px', textAlign: 'center' }}>
          ...
          <div style={{ background: '#2196F3', color: 'white', padding: '12px 16px', borderRadius: '8px', textAlign: 'center' }}>
          ...
          <div style={{ background: '#dc3545', color: 'white', padding: '12px 16px', borderRadius: '8px', textAlign: 'center' }}>
      ...
      {forecastData?.forecast_type === 'synthetic' && (
        <div style={{ padding: '12px 16px', background: '#fff3cd', borderRadius: '8px', border: '1px solid #ffc107', fontSize: '12px', color: '#856404', marginBottom: '24px' }}>
      ...
      <div style={{ marginBottom: '24px' }}>
        <MetricsPanel matchRate={cashPosition?.match_rate || 0} totalSettlements={totalSettlements} matched={totalMatches} exceptions={totalExceptions} dataSource={dataSource} />
      </div>

      {/* Match and Exception Summary */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '24px' }}>
        <MatchSummaryCard count={totalMatches} amount={cashPosition?.current_cash || 0} matches={matches.slice(0, 5)} onViewAll={onViewMatches} />
        <ExceptionSummaryCard count={totalExceptions} amount={cashPosition?.pending_settlements || 0} exceptions={exceptionList.slice(0, 5)} onViewAll={onViewExceptions} />
      </div>

      {/* Footer Stats */}
      <div style={{ marginTop: '20px', padding: '16px', background: '#f8f9fa', borderRadius: '8px', display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px', fontSize: '13px', color: '#666' }}>
        <div><strong>Last Updated:</strong> {new Date().toLocaleString()}</div>
        <div><strong>Data Source:</strong> {dataSource === 'sample' ? 'Sample Data' : 'Uploaded CSV'}</div>
        <div><strong>CashSight</strong> — AI Finance Controller</div>
      </div>
    </div>
  )
}